...
export function SellNewQuote() {
  ...
  const queueItemsForReview = (items: { description: string; sku: string; qty: number; unit: string; unitCost: number; margin: number; unitPrice: number }[], source: ReviewLineItem['source']) => {
    const converted: ReviewLineItem[] = items.map(i => ({
      id: Math.random().toString(36).slice(2),
      ...i,
      source,
      status: 'pending',
    }));
    setReviewLines(prev => [...converted, ...prev]);
  };

  const acceptReviewLine = (id: string) => {
    const match = reviewLines.find(line => line.id === id);
    if (!match) return;

    const { source: _source, status: _status, ...acceptedLine } = match;
    setLines(prev => [...prev, acceptedLine]);
    setReviewLines(prev => prev.filter(line => line.id !== id));
  };

  const acceptAllReviewLines = () => {
    const accepted = reviewLines.map(({ source: _source, status: _status, ...line }) => line);
    if (accepted.length === 0) return;
    setLines(prev => [...prev, ...accepted]);
    setReviewLines([]);
    toast.success(`Added ${accepted.length} reviewed item${accepted.length !== 1 ? 's' : ''} to quote`);
  };

  const dismissReviewLine = (id: string) => {
    setReviewLines(prev => prev.filter(line => line.id !== id));
  };
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-[1200px] mx-auto space-y-6">
          ...
          {aiSuggestion && customer && (
            <AIInsightCard title="MirrorWorks Agent suggestion">
          ...
          <QuoteAssistantBar
            customer={customer}
            lines={lines}
            onQueueLinesForReview={(items) => queueItemsForReview(items, 'assistant')}
                ...
                <Card>
                  <div className="flex items-center justify-between border-b border-[var(--border)] p-4">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[var(--neutral-500)]">
                        Review assistant-generated draft lines before accepting them into the quote.
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className="border-0 bg-[var(--mw-yellow-50)] text-[var(--mw-yellow-700)]">
                        {reviewLines.length} pending
                      </Badge>
                      <Button
                        size="sm"
                        className="bg-[var(--mw-yellow-400)] hover:bg-[var(--mw-yellow-500)] text-primary-foreground"
                        onClick={acceptAllReviewLines}
                      >
                  ...
                  <div className="divide-y divide-[var(--border)]">
                    {reviewLines.map((line) => (
                      <div key={line.id} className="grid gap-4 p-4 md:grid-cols-[1.5fr_110px_120px_120px_auto] md:items-center">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="truncate text-sm font-medium text-foreground">{line.description || 'Untitled item'}</p>
                            <Badge variant="outline" className="text-[10px] uppercase tracking-wide">
                              {line.source}
                            </Badge>
                          </div>
                          <p className="mt-1 text-xs text-[var(--neutral-500)]">
                            {line.sku || 'No SKU'} · {line.unit} · margin {line.margin.toFixed(1)}%
                          </p>
                        </div>
                        <div className="text-sm">
                          <p className="text-[10px] uppercase tracking-wide text-[var(--neutral-500)]">Qty</p>
                          <p className="font-medium tabular-nums text-foreground">{line.qty}</p>
                        </div>
                        <div className="text-sm">
                          <p className="text-[10px] uppercase tracking-wide text-[var(--neutral-500)]">Cost</p>
                          <p className="font-medium tabular-nums text-[var(--neutral-600)]">${fmtCurrency(line.unitCost)}</p>
                        </div>
                        <div className="text-sm">
                          <p className="text-[10px] uppercase tracking-wide text-[var(--neutral-500)]">Price</p>
                          <p className="font-medium tabular-nums text-foreground">${fmtCurrency(line.unitPrice)}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => toast('Edit this draft in the line items table after accepting it')}
                          >
                          ...
                          <Button
                            size="sm"
                            className="bg-[var(--mw-yellow-400)] hover:bg-[var(--mw-yellow-500)] text-primary-foreground"
                            onClick={() => acceptReviewLine(line.id)}
                          >
                            <Check className="mr-1 h-3.5 w-3.5" />
                            Accept
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => dismissReviewLine(line.id)}>
                            Dismiss
                          </Button>
                        </div>
                      </div>
