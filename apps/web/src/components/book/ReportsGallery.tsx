...
const ReportCard = ({ icon: Icon, title, desc, borderColor, badge, ai }: any) => (
  <Card variant="flat" className={cn("rounded-lg border border-[var(--border)] p-6 transition-colors duration-[var(--duration-medium1)] ease-[var(--ease-standard)]", borderColor && `border-l-[3px] ${borderColor}`)}>
    <Icon className="w-8 h-8 text-foreground mb-3" />
    <div className="flex items-center gap-2 mb-1">
      <h3 className="text-sm text-foreground font-medium">{title}</h3>
      {ai && <Badge className="rounded-full text-[10px] px-2 py-0 border-0 bg-[var(--mw-purple)]/15 text-[var(--mw-purple)]">Agent</Badge>}
    </div>
