...
import { peopleModulePermissionLabels } from './people-data';
import type { ModuleKey, PermissionKey, PermissionSet, ScopeValue } from '@mirrorworks/contracts';
...
export function PermissionGrid({ module, resolved, grantedBy }: PermissionGridProps) {
  const entries = peopleModulePermissionLabels[module].filter(entry => entry.key !== 'documents.scope');
  const rows: { key: PermissionKey; label: string }[] = [
    { key: 'documents.scope', label: 'Document visibility' },
    ...entries.map(e => ({ key: e.key as PermissionKey, label: e.label })),
  ];
