...
import type { ModuleKey, PermissionDefinition } from '@mirrorworks/contracts';

interface GroupsTabProps {
  onOpenGroupDetail: (group: PeopleGroupView) => void;
}
...
export function GroupsTab({ onOpenGroupDetail }: GroupsTabProps) {
  ...
  const grouped = useMemo(
    () =>
      moduleOrder.reduce<Record<ModuleKey, PeopleGroupView[]>>(
        (acc, moduleKey) => ({ ...acc, [moduleKey]: peopleGroups.filter(group => group.module === moduleKey) }),
        { sell: [], plan: [], make: [], ship: [], book: [], buy: [], control: [] },
      ),
    [],
  );

  const currentPermissionLabels = peopleModulePermissionLabels[activeModule].filter(
    entry => entry.key !== 'documents.scope',
  );
      ...
      <div className="space-y-4">
        {grouped[activeModule].map(group => {
          ...
          const enabledCount = currentPermissionLabels.filter(entry => {
            const val = group.permissionSet[entry.key];
            return val === true || val === 'all';
          }).length;
          const scopeVal = group.permissionSet['documents.scope'];
          if (scopeVal === 'all') {
...
function PermissionSection({
  ...
  group: PeopleGroupView;
  section: 'actions' | 'admin';
  labels: PermissionDefinition[];
}) {
