  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-[var(--mw-purple)]">MirrorWorks Agent suggestions</span>
      </div>
        ...
        {suggestions.map((s) => (
          ...
          >
            <MirrorWorksAgentCard
              title={s.principleTag}
              suggestion={s.message}
              tone="opportunity"
              primaryAction={{ label: 'Apply', onClick: () => onApply(s.id) }}
              secondaryAction={{ label: 'Dismiss', onClick: () => onDismiss(s.id) }}
              detailContent={<p>{s.principle}</p>}
              evidenceLevel="expandable"
              detailLabel="Lean principle"
            />
          </motion.div>
        ))}
      </AnimatePresence>
      ...
      {suggestions.length === 0 && (
        <p className="text-xs text-[var(--neutral-500)] text-center py-4">
          No suggestions yet. Add more nodes and connections, then ask MirrorWorks Agent to review the flow.
        </p>
      )}
    </div>
  );
...
export function ControlProcessBuilder() {
  ...
  useEffect(() => {
    if (searchParams.get('from') === 'factory') {
      try {
        const raw = readStorageValue('mw-factory-to-process', 'session');
        if (raw) {
          const parsed = JSON.parse(raw) as FactoryRefElement[];
          setFactoryLayer(parsed);
          removeStorageValue('mw-factory-to-process', 'session');
  ...
  const handleAnalyzeAI = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      toast.success('MirrorWorks Agent review complete — 4 suggestions found');
    }, 1600);
  }, []);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="h-12 border-b border-[var(--border)] bg-card flex items-center px-4 gap-3 flex-shrink-0">
        ...
        >
          <Sparkles className="w-3.5 h-3.5" /> Analyze with Agent
        </Button>
      </div>
