...
export function FloorHome() {
  ...
  const pendingResumeWorkOrderId = useFloorExecutionStore(
    (state) => state.pendingResumeWorkOrderId
  );
  ...
  // After an operator switch, keep the station but re-open the same work
  // order once the next operator has clocked in.
  useEffect(() => {
    if (session.operatorId && pendingResumeWorkOrderId) {
      navigate(`/floor/run/${pendingResumeWorkOrderId}`, { replace: true });
    }
  }, [navigate, pendingResumeWorkOrderId, session.operatorId]);
