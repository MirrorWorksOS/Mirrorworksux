...
function loadProducts(): Product[] {
  const parsed = readStorageJson<Product[] | null>(STORAGE_KEY, null);
  if (Array.isArray(parsed) && parsed.length > 0) {
    ...
    if (JSON.stringify(migrated) !== JSON.stringify(parsed)) {
      writeStorageJson(STORAGE_KEY, migrated);
    }
    return migrated;
  }
  const templates = productTemplates.map((t) => ({ ...t }));
  writeStorageJson(STORAGE_KEY, templates);
  return templates;
}

function saveProducts(products: Product[]) {
  writeStorageJson(STORAGE_KEY, products);
}
