...
| Font | Weights | Use |
|---|---|---|
| Roboto | 300 (Light), 400 (Regular), 500 (Medium), 700 (Bold), 900 (Black) | All UI text including financial values, job IDs, codes |

**Single font family.** Only Roboto. Geist, JetBrains Mono, Inter, Roboto Mono are all deprecated and must not be used.

For financial values, IDs, and tabular data, use `font-variant-numeric: tabular-nums` (Tailwind: `tabular-nums`) to get fixed-width numerals within Roboto. No monospace font needed.
...
1. Financial values, currency, totals → Roboto with `tabular-nums` at applicable size
2. Job numbers, IDs, codes → Roboto with `tabular-nums` at applicable size
...
- Job numbers/IDs in Roboto with `tabular-nums`
...
### Book (Finance)
- All financial values in Roboto `tabular-nums`
