...
#### ✅ What Works Well:
- **MW Design System Adherence:** Excellent use of `#FFCF4B` yellow, `Roboto Mono` for financials
...
// ✅ Good: Roboto Mono for financial figures
...
style={{ 
  ...
  fontWeight: 500, 
  color: row.variance < 0 ? '#36B37E' : '#DE350B' 
}}
...
2. **Typography:**
   - ✅ Roboto Mono for financial figures (perfect for tabular data alignment)
   - ✅ Geist for UI text (clean, modern)
   - ✅ JetBrains Mono for code-like IDs (job numbers, PO refs)

3. **Card Styling:**
   - ✅ Sharp corners (`rounded-lg` = 8px, not rounded-xl)
   - ✅ Borders: `border-[#E5E5E5]` (1px solid)
   - ✅ Shadows: `shadow-[0_1px_2px_0_rgba(0,0,0,0.05)]` (subtle)
...
1. **Card Border Radius:**
   - BudgetOverview.tsx: Uses `rounded-lg` ✅
   - JobCostDetail.tsx: Uses `rounded-xl` ❌ (line 66)
   - **Fix:** Standardize to `rounded-lg` (8px) per MW design system

2. **Font Family Declarations:**
   - Some use inline styles: `style={{ fontFamily: 'Roboto Mono, monospace' }}`
   - Should use Tailwind classes where possible: `font-mono`
...
4. **Hover State Transitions:**
   ...
   - Should use: `transition-colors duration-200 ease-[cubic-bezier(0.0,0.0,0.2,1.0)]`
   - **Fix:** Apply consistent 200ms ease-out transitions (per recent animation update)
...
- ✅ Roboto Mono usage for financial data is spot-on
...
<Card className="bg-white rounded-lg border border-[#E5E5E5] p-6">
  <h3 className="font-['Geist:SemiBold',sans-serif] text-[16px] mb-4">Budget Breakdown</h3>
    ...
    {budgets.map(budget => (
      <tr key={budget.id} className="border-b border-[#F5F5F5] hover:bg-[#FFFBF0] cursor-pointer">
        <td className="px-4 py-3">
          <a href={`/book/job-costs/${budget.id}`} className="text-[#0052CC] font-mono text-sm hover:underline">
            {budget.name}
          </a>
        </td>
        <td className="px-4 py-3">
        ...
        <td className="px-4 py-3 text-sm font-mono font-medium">${budget.budgeted.toLocaleString()}</td>
        <td className="px-4 py-3 text-sm font-mono font-medium">${budget.actual.toLocaleString()}</td>
        <td className="px-4 py-3 text-sm font-mono" style={{ color: budget.variance < 0 ? '#36B37E' : '#DE350B' }}>
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#737373] font-mono">{budget.utilisation}%</span>
          </div>
        </td>
        <td className="px-4 py-3">
          <Badge className={cn(
