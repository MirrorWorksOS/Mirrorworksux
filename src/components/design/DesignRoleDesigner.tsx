  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {roles.map(role => (
          <Card key={role.name} className="bg-white border border-[#E5E5E5] rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: role.color }}>
                <Shield className="w-5 h-5 text-white" />
              </div>
              <div>
                <h3 className="font-['Geist:SemiBold',sans-serif] text-[14px] font-semibold text-[#0A0A0A]">{role.name}</h3>
                <p className="text-xs text-[#737373]">{role.description}</p>
              </div>
            </div>
            <Button variant="outline" size="sm" className="w-full border-[#E5E5E5]">Configure</Button>
          </Card>
        ))}
      </div>
    </div>
