    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {dataTypes.map(type => {
          ...
          return (
            <Card key={type.key} className="bg-white border border-[#E5E5E5] rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-['Geist:SemiBold',sans-serif] text-[14px] font-semibold text-[#0A0A0A]">{type.label}</h3>
                    <p className="text-xs text-[#737373] mt-1">
                      {type.imported} / {type.total} records
                    </p>
                  </div>
                </div>
                {type.status === 'complete' && <CheckCircle2 className="w-5 h-5 text-[#36B37E]" />}
      ...
      <div className="flex items-center justify-between pt-6 border-t border-[#E5E5E5]">
        <Button variant="outline" className="border-[#E5E5E5]" disabled={activeStep === 1}>
          ← Previous
        </Button>
        <div className="flex items-center gap-2">
          {[1, 2, 3].map(step => (
            <div key={step} className={cn(
              ...
              step === activeStep ? "bg-[#FFCF4B]" : step < activeStep ? "bg-[#36B37E]" : "bg-[#E5E5E5]"
            )} />
          ))}
        </div>
