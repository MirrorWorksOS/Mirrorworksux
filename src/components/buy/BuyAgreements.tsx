...
const STATUS_CONFIG: Record<string, { bg: string; text: string; label: string; barColor: string }> = {
  active:      { bg: 'bg-[#F5F5F5]', text: 'text-[#1A2732]', label: 'Active',     barColor: '#FFCF4B' },
  ...
  return (
    <motion.div initial="initial" animate="animate" variants={animationVariants.stagger} className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[32px] tracking-tight text-[#1A2732]">Blanket agreements</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <Card key={s.label} className="bg-white border border-[var(--border)] rounded-2xl p-5">
            ...
            <p className="text-[22px]  font-semibold text-[#1A2732]">{s.value}</p>
              ...
              <Card className="bg-white border border-[var(--border)] rounded-2xl p-5 hover:shadow-md transition-shadow duration-150 cursor-pointer group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm  font-medium text-[#737373]">{agr.agreementNumber}</span>

                      <Badge className={cn('border-0 text-xs rounded-full px-2 py-0.5', cfg.bg, cfg.text)}>{cfg.label}</Badge>
                    </div>
                    <h3 className="text-[15px] font-semibold text-[#1A2732]">{agr.supplier}</h3>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[18px]  font-semibold text-[#1A2732]">${agr.value.toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 mb-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className=" text-[#1A2732] font-medium">{spendPct.toFixed(0)}%</span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm pt-3 border-t border-[var(--border)]">
                  ...
                  <div>
                    ...
                    <p className=" font-medium text-[#FF8B00]">${agr.committed.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-[#737373] mb-0.5">Remaining</p>
                    <p className={cn('font-[\'Roboto_Mono\',monospace] font-medium', remaining <= 0 ? 'text-[#EF4444]' : remaining < agr.value * 0.1 ? 'text-[#FF8B00]' : 'text-[#1A2732]')}>
