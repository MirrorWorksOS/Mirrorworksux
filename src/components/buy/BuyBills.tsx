...
const STATUS_CONFIG: Record<string, { bg: string; text: string; label: string; icon: any }> = {
  matched:  { bg: 'bg-[#F5F5F5]', text: 'text-[#1A2732]', label: 'Matched',  icon: CheckCircle2 },
    ...
    <div className="flex items-center gap-1.5">
      {dots.map(d => (
        <div key={d.label} className="flex flex-col items-center gap-0.5" title={d.label}>
          <div className={cn('w-2 h-2 rounded-full', d.ok ? 'bg-[#1A2732]' : 'bg-[#EF4444]')} />
  ...
  return (
    <motion.div initial="initial" animate="animate" variants={animationVariants.stagger} className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Matched',         value: BILLS.filter(b => b.status === 'matched').length,  sub: `$${totals.matched.toLocaleString()}`, bg: 'bg-[#F5F5F5]', text: 'text-[#1A2732]' },
            ...
            {filtered.map(bill => {
              ...
              return (
                <tr key={bill.id} onClick={() => setSelected(bill)}
                  ...
                  <td className="px-4 text-sm  font-medium text-[#1A2732]">{bill.billNumber}</td>
                  <td className="px-4 text-sm text-[#1A2732] font-medium">{bill.supplier}</td>
                  ...
                  <td className="px-4 text-sm  text-[#1A2732]">{bill.poNumber}</td>
                  <td className="px-4">
        ...
        <Sheet open onOpenChange={() => setSelected(null)}>
          <SheetContent className="w-[480px] sm:max-w-[480px] p-0 overflow-y-auto border-l border-[var(--border)]">
            <SheetHeader className="p-6 pb-4 border-b border-[var(--border)]">
              <SheetTitle className="text-[18px] font-medium  text-[#1A2732]">{selected.billNumber}</SheetTitle>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={row.label} className="flex items-center justify-between p-3 bg-[#F5F5F5] rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className={cn('w-6 h-6 rounded-full flex items-center justify-center', row.ok ? 'bg-[#F5F5F5]' : 'bg-[#FEE2E2]')}>
                          {row.ok
                            ? <CheckCircle2 className="w-4 h-4 text-[#1A2732]" />
                            : <X className="w-4 h-4 text-[#EF4444]" />
                          }
                        </div>
                        <span className="text-sm text-[#1A2732] font-medium">{row.label}</span>
                      </div>
                      <span className="text-sm  text-[#737373]">{row.ref}</span>
                    </div>
                  ))}
                </div>
              </div>
