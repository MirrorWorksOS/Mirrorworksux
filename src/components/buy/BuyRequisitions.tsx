...
const getStatusBadge = (status: ReqStatus) => {
  switch (status) {
    ...
    case 'approved': return { bg: 'bg-[#F5F5F5]', text: 'text-[#1A2732]', label: 'Approved', dot: '#1A2732' };
    case 'rejected': return { bg: 'bg-[#FEE2E2]', text: 'text-[#EF4444]', label: 'Rejected', dot: '#EF4444' };
    case 'converted': return { bg: 'bg-[#F5F5F5]', text: 'text-[#1A2732]', label: 'Converted', dot: '#1A2732' };
  }
};
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredReqs.map((req, idx) => {
                ...
                return (
                  <tr key={req.id} className={cn("border-b border-[var(--border)] h-14 hover:bg-[var(--accent)] cursor-pointer transition-colors", idx % 2 === 1 && "bg-[#F5F5F5]")}>
                    <td className="px-4">
                      <a href={`/buy/requisitions/${req.id}`} className="text-[#1A2732]  text-sm font-medium hover:underline flex items-center gap-1">
                        {req.reqNumber}
                        <ExternalLink className="w-3 h-3" />
                      </a>
                    </td>
                    <td className="px-4 text-sm text-[#1A2732]">{req.requestor}</td>
                    ...
                    <td className="px-4">
                      {req.status === 'submitted' && (
                        <div className="flex items-center gap-1">
                          <button className="p-1 hover:bg-[#F5F5F5] rounded transition-colors" title="Approve">
