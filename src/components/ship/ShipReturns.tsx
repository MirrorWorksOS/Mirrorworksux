...
const rStatusConfig: Record<RStatus, { label: string; badge: string; text: string; dot: string }> = {
  ...
  received:   { label: 'Received',   badge: 'bg-[#E3FCEF]', text: 'text-[#36B37E]', dot: '#36B37E' },
  refunded:   { label: 'Refunded',   badge: 'bg-[#E3FCEF]', text: 'text-[#36B37E]', dot: '#36B37E' },
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-[32px] tracking-tight text-[#0A0A0A]">Returns</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {/* Table */}
        <div className="lg:col-span-3 bg-white rounded-lg border border-[#E5E5E5] overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {RMAS.map(r => {
                ...
                return (
                  <tr key={r.id} className="border-b border-[#F5F5F5] h-14 hover:bg-[#FFFBF0] cursor-pointer transition-colors" onClick={() => setSelected(r)}>
                    <td className="px-4 py-3 text-sm font-['Roboto_Mono',monospace] font-medium text-[#0A0A0A]">{r.id}</td>
                    <td className="px-4 py-3 text-sm text-[#0A0A0A]">{r.customer}</td>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg p-5 border border-[#E5E5E5]">
            ...
            <div className="text-3xl text-[#0A0A0A] mt-1 font-['Roboto_Mono',monospace] font-semibold">12</div>
          </div>
          <div className="bg-white rounded-lg p-5 border border-[#E5E5E5]">
          ...
          <div className="bg-white rounded-lg p-5 border border-[#E5E5E5]">
            ...
            <div className="text-3xl text-[#0A0A0A] mt-1 font-['Roboto_Mono',monospace] font-semibold">
          ...
          <div className="bg-white rounded-lg p-5 border border-[#E5E5E5]">
            ...
            <div className="text-3xl text-[#0A0A0A] mt-1 font-['Roboto_Mono',monospace] font-semibold">2.8%</div>
          </div>
        </div>
      </div>
              ...
              <>
                ...
                <div className="px-6 py-6 space-y-6">
                  <div className="bg-[#FAFAFA] rounded-lg p-4 space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-[#737373]">Order</span><span className="font-['Roboto_Mono',monospace] font-medium">{selected.order}</span></div>
                    <div className="flex justify-between"><span className="text-[#737373]">Items</span><span className="font-['Roboto_Mono',monospace] font-medium">{selected.items}</span></div>
                        ...
                        return (
                          <div key={stage} className="flex items-center gap-4 relative py-2.5">
                            <div className={cn(
                              ...
                              done ? 'bg-[#0A0A0A] border-[#0A0A0A]' : 'bg-white border-[#E5E5E5]'
                            )} />
                            <span className={cn('text-sm', done ? 'text-[#0A0A0A] font-medium' : 'text-[#A3A3A3]')}>
                              {stageCfg.label}
                            </span>
                          </div>
                        );
                  ...
                  <div className="space-y-2 pt-2">
                    {selected.status === 'pending' && (
                      <button className="w-full h-11 rounded-lg text-sm bg-[#0A0A0A] text-white hover:bg-[#2C2C2C] transition-colors font-medium">
                        Approve return
                      </button>
                    )}
                    ...
                    <button className="w-full h-11 rounded-lg text-sm border border-[#E5E5E5] text-[#0A0A0A] hover:bg-[#F5F5F5] transition-colors font-medium">
                      Contact customer
                    </button>
                  </div>
                </div>
