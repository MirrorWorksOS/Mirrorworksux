...
const donutData = [
  { name: 'Raw Materials', value: 145600, color: '#0052CC' },
  { name: 'Work in Progress', value: 89200, color: '#FACC15' },
  { name: 'Finished Goods', value: 67800, color: '#36B37E' },
];
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto max-w-[1200px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-[#1A2732] rounded-lg shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] border border-[#1A2732] p-6">
          <div className="text-xs tracking-wider text-white/70 mb-2" style={{ fontWeight: 500 }}>TOTAL INVENTORY VALUE</div>
          <div className="text-[24px] tracking-tight text-white" style={{ fontFamily: 'Roboto Mono, monospace', fontWeight: 500 }}>$302,600</div>
        </Card>
      </div>
      ...
      <Card className="bg-white rounded-lg shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] border border-[var(--border)] overflow-hidden">
              ...
              <tbody>
                {rawMaterials.map((r, i) => (
                  <tr key={r.sku} className={cn("border-b border-[#F5F5F5] h-14 hover:bg-[var(--accent)]", i % 2 === 1 && "bg-[#F5F5F5]")}>
                    ...
                    <td className="px-4 text-sm text-right" style={{ fontFamily: 'Roboto Mono, monospace' }}>{r.unit}</td>
                    <td className="px-4 text-sm text-right" style={{ fontFamily: 'Roboto Mono, monospace', fontWeight: 500 }}>${r.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="px-4 text-sm text-[#525252]">{r.location}</td>
                    <td className="px-4 text-sm text-[#525252]">{r.lastMove}</td>
                    <td className="px-4"><Badge className={cn("rounded-full text-[11px] px-2 py-0.5 border-0", ageStyles[r.age])}>{r.age}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab !== 'Raw Materials' && (
          <div className="p-8 text-center text-sm text-[#A3A3A3]">No {activeTab.toLowerCase()} data to display.</div>
        )}
      </Card>
    </div>
  );
